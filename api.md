API
이곳에서는 API 요청의 기본적인 정보를 안내합니다. 프로그래밍
...
버전
API 요청을 보낼 때는 다음과 같이 사용합니다. https://koreanbots.dev/api/v{version}
...
0	사용 불가
레이트리밋
레이트리밋(RateLimit)은 로그인 토큰 기준이 아닌, IP 기준으로 적용됩니다.
레이트리밋은 클라이언트와 동일 IP로 접속할 경우 공유하게 됩니다.

효율적인 리소스 관리를 위한 일정시간의 요청 제한 시간입니다.

만약 제한된 요청 횟수보다 더 많이 요청할 경우 429의 STATUSCODE(상태코드)를 반환합니다.
...
ENDPOINT	RATELIMIT
/* (Global)	180회/분
특정 엔드포인트들은 별도의 레이트리밋이 있습니다. 레이트리밋 헤더를 통해 정보를 확인할 수 있습니다.
