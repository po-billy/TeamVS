...
owners	User[]	봇의 소유자 (단, 소유한 봇은 아이디만 표시됩니다)
flags	integer	봇의 플래그
lib	string	봇의 라이브러리
...
category	Category[]	봇의 카테고리
vanity	?string	봇의 VANITY URL
bg	?string	봇의 배경 이미지 주소
banner	?string	봇의 배너 이미지 주소
status	?Status	봇의 유저 상태
state	State	한국 디스코드봇 리스트에서의 봇의 상태
봇 카테고리 (Category)
관리
뮤직
전적
...
해당 엔드포인트는 추가적인 레이트리밋이 적용됩니다. 따라서 3분에 3개의 요청을 보내실 수 있습니다
